...
const buildGroupSites = (options: SiteOption[]): {
  ...
  const groups: GroupedSite[] = Array.from(grouped.entries()).map(
    ([label, sites]) => ({ id: label, label, sites })
  );
...
export default function SiteDropdownGrouped({
  ...
  const [submenuMaxWidth, setSubmenuMaxWidth] = React.useState<number>(
    SUBMENU_PREFERRED_WIDTH
  );
  const pickSubmenuSide = (el: HTMLElement | null) => {
    ...
    // If preferred width overflows on right, prefer opening to the left.
    ...
    >
      {({ open, selected, options: rawOptions, getButtonProps, getMenuProps, getItemProps }) => {
        ...
        const fallbackOption =
          selected ??
          siteOptionList.find((opt) => opt.value === value) ??
          siteOptionList[0];
              ...
              {groups.map((group) => (
                  <div key={group.id} className="relative">
                    ...
                    >
                      ...
                      <i className="material-icons text-[18px] leading-none text-gray-500">
                        chevron_right
                      </i>
                    </button>
