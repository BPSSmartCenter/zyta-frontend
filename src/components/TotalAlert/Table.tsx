...
import { CAMERA_OPTIONS, EVENT_OPTIONS, type AlertRow } from "./totalAlert.constant";
...
const relHours = (iso: string, t: (k: string, o?: any) => string) => {
  const diffH = Math.max(0, Math.round((Date.now() - new Date(iso).getTime()) / 36e5));
  return diffH === 0 ? t("relative.justNow", { defaultValue: "just now" }) : t("relative.hoursAgo", { count: diffH, defaultValue: "{{count}} hours ago" });
};
const extractCameraNumber = (s?: string | null) => (s?.match(/(\d+)/)?.[1] ? parseInt(s.match(/(\d+)/)![1], 10) : null);
const useCameraLabelT = () => {
  const { t } = useTranslation("alert");
  return (label?: string | null) => {
    const num = extractCameraNumber(label);
    return num != null ? t("camera.numbered", { number: num, defaultValue: `Camera ${num}` }) : label ?? "";
  };
};
const isAllOption = (opt: { value?: string; label?: string } | undefined, kind: "camera" | "event") => {
  if (!opt) return false;
  const v = String(opt.value ?? "").toLowerCase().trim();
  const l = String(opt.label ?? "").toLowerCase().trim();
  if (v === "all") return true;
  if (kind === "camera") return (l.includes("all") && l.includes("camera")) || l.includes("กล้องทั้งหมด");
  return (l.includes("all") && l.includes("event")) || l.includes("เหตุการณ์ทั้งหมด");
};
const getPic = (n: any) =>
  n?.screenshot ?? n?.screenShot ?? n?.screenshotUrl ?? n?.thumbnail ?? n?.img ?? n?.image ?? n?.picture ?? "";
...
const parseForcedEvent = (s?: string | null): PrimaryEvent => {
  ...
  return (["motion", "fall", "fire", "offline", "sleep"] as const).includes(v as any) ? (v as PrimaryEvent) : "motion";
};
...
export default function Table() {
  ...
  React.useEffect(() => {
    const match = EVENT_OPTIONS.find((o) => String(o.value ?? "").toLowerCase() === forcedEvent);
  ...
  const rowsFiltered: AlertRow[] = React.useMemo(() => {
    return rowsAll
      ...
      .filter((r) => (cameraFilter === "all" ? true : r.cameraLabel === cameraFilter))
      .filter((r) => {
        if (!date) return true;
      ...
      .filter((r) =>
        q.trim()
          ? `${r.cameraLabel} ${r.event} ${r.cameraName}`.toLowerCase().includes(q.toLowerCase())
          : true
      );
  ...
  React.useEffect(() => { setPage(1); }, [cameraFilter, eventFilter, date, q, forcedEvent]);
    ...
    <div className="p-6">
      ...
      <div className="px-1">
        <div className="flex items-start gap-3">
          <h2 className="text-[27px] font-bold">{t("sitesAlertsTitle", { defaultValue: "Sites Alerts Table" })}</h2>
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-wrap items-center gap-3">
          {/* Camera */}
          <Dropdown options={CAMERA_OPTIONS} value={cameraFilter} onChange={setCameraFilter}>
            {({ open, selected, getButtonProps, getMenuProps, getItemProps, options }) => {
              const selectedLabel = isAllOption(selected, "camera") ? t("filters.cameraAll", { defaultValue: "All CAMERA" }) : tCamera(selected?.label);
              return (
                <div className="relative">
                  <button {...getButtonProps({ className:"h-[40px] min-w-[130px] rounded-md border border-gray-300 bg-white px-3 text-[14px] text-gray-800 font-semibold flex items-center justify-between gap-2" })}>
                    <span className="truncate">{selectedLabel || t("filters.cameraAll", { defaultValue: "All CAMERA" })}</span>
                    <i className="material-icons leading-none">{open ? "arrow_drop_up" : "arrow_drop_down"}</i>
                  </button>
                  {open && (
                    <div {...getMenuProps({ className:"absolute z-50 mt-1 w-full rounded-lg border border-gray-200 bg-white p-1 shadow-lg " })}>
                      {options.map((opt) => {
                        const label = isAllOption(opt, "camera") ? t("filters.cameraAll", { defaultValue: "All CAMERA" }) : tCamera(opt.label);
                        return (
                          <button key={String(opt.value ?? opt.label)} {...getItemProps(opt, { className:"w-full text-left rounded-md px-3 py-2 text-[14px] hover:bg-gray-100 cursor-pointer" })}>
                            {label}
                          </button>
                        );
                      })}
          ...
          {/* Event */}
          <Dropdown options={EVENT_OPTIONS} value={eventFilter} onChange={setEventFilter}>
            {({ open, selected, getButtonProps, getMenuProps, getItemProps, options }) => {
              const { t } = useTranslation("alert");
              const selectedLabel = isAllOption(selected, "event")
                ? t("filters.eventAll", { defaultValue: "All Event" })
                : t(`events.${String(selected?.label ?? "").toLowerCase()}`, { defaultValue: String(selected?.label ?? "") });
              return (
                <div className="relative">
                  <button {...getButtonProps({ className:"h-[40px] min-w-[130px] rounded-md border border-gray-300 bg-white px-3 font-semibold text-[14px] text-gray-800 flex items-center justify-between gap-2" })}>
                    <span className="truncate">{selectedLabel || t("filters.eventAll", { defaultValue: "All Event" })}</span>
                    <i className="material-icons leading-none">{open ? "arrow_drop_up" : "arrow_drop_down"}</i>
                  </button>
                  {open && (
                    <div {...getMenuProps({ className:"absolute z-50 mt-1 w-full rounded-lg border border-gray-200 bg-white p-1 shadow-lg" })}>
                      {options.map((opt) => {
                        const label = isAllOption(opt, "event")
                          ? t("filters.eventAll", { defaultValue: "All Event" })
                          : t(`events.${String(opt.label ?? "").toLowerCase()}`, { defaultValue: String(opt.label ?? "") });
                        return (
                          <button key={String(opt.value ?? opt.label)} {...getItemProps(opt, { className:"w-full text-left rounded-md px-3 py-2 text-[14px] hover:bg-gray-100 cursor-pointer" })}>
                            {label}
                          </button>
        ...
        {/* Search */}
        <SearchInput value={q} onChange={setQ} placeholder={t("searchPlaceholder", { defaultValue: "Search events" })} className="w-full md:w-[340px]" />
      </div>
      ...
      <div className="mt-4 overflow-x-auto md:overflow-hidden rounded-lg bg-white">
        <div className="inline-block w-full min-w=[750px] md:min-w-0 align-middle">
          <div className="grid grid-cols-[48px_1.2fr_1fr_120px_1fr_1fr_1.4fr] place-items-center px-4 py-3 text-[12px] font-medium text-gray-500 text-center bg-gray-100">
            ...
            <div>{t("table.headers.cameraName", { defaultValue: "Camera Name" })}</div>
            <div>{t("table.headers.status", { defaultValue: "Status" })}</div>
            <div>{t("table.headers.timestamp", { defaultValue: "Timestamp" })}</div>
          </div>
          ...
          {pageRows.map((r) => {
            ...
            return (
              <div key={r.id}
                className={["grid grid-cols-[48px_1.2fr_1fr_120px_1fr_1fr_1.4fr] place-items-center px-4 py-3 text-[14px] text-center","border-t border-gray-100",isSel ? "bg-blue-50" : "bg-white"].join(" ")}>
                <div className="flex items-center justify-center gap-2">
                  <input id={checkboxId} type="checkbox" checked={isSel} onChange={(e) => setSelectedId(e.target.checked ? r.id : null)}
                    className="size-5 rounded-md focus:ring-0 checked:border-cyan checked:bg-cyan hover:cursor-pointer" aria-labelledby={cameraCellId}/>
                  <label htmlFor={checkboxId} className="sr-only">
                    {t("a11y.selectRow", { camera: r.cameraLabel, defaultValue: "Select {{camera}}" })}
                  </label>
                </div>
                <div id={cameraCellId} className="text-cyan font-inter underline cursor-pointer">
                  {tCamera(r.cameraLabel)}
                </div>
                <div className="text-gray-700">{t(`events.${r.event}`, { defaultValue: r.event })}</div>
                <div className="h-[80px] w-[100px] overflow-hidden rounded-md">
                  {r.picture ? <img src={r.picture} alt="" className="h-full w-full object-cover" /> : <div className="h-full w-full bg-gray-200" />}
                </div>
                <div className="text-gray-700">{r.cameraName}</div>
                <div className={["font-semibold", r.status === "UNRESOLVED" ? "text-red-600" : "text-green-600"].join(" ")}>
        ...
        <div className="mt-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))}
              className={["h-[34px] rounded-md border px-3 text-[14px]", page <= 1 ? "text-gray-400 border-gray-200 cursor-not-allowed" : "border-gray-300 cursor-pointer"].join(" ")}>
              {t("pager.prev", { defaultValue: "Previous" })}
            </button>
            <button type="button" onClick={() => setPage((p) => Math.min(pageCount, p + 1))}
              className={["h-[34px] rounded-md border px-3 text-[14px]", page >= pageCount ? "text-gray-400 border-gray-200 cursor-not-allowed" : "border-gray-300 cursor-pointer"].join(" ")}>
